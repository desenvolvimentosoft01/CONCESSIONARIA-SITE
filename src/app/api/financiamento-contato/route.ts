...
function buildTemplate(params: {
  ...
  const { nome, email, telefone, valorVeiculo, mensagem, dataHora, ip } = params;
...
<head>
  ...
  <title>Solicitação de Financiamento — ${DADOS_LOJA.nome}</title>
</head>
        ...
        <table width="600" cellpadding="0" cellspacing="0" border="0" style="max-width:600px;width:100%;background-color:#ffffff;border-radius:12px;overflow:hidden;box-shadow:0 8px 30px rgba(0,0,0,0.12);">
          ...
          <tr>
            <td style="padding:28px 30px 0;">
              <table width="100%" cellpadding="0" cellspacing="0" style="background-color:#fff8ee;border-left:4px solid #c5a059;border-radius:4px;padding:16px 20px;">
                <tr>
                  <td>
                    <p style="margin:0 0 4px 0;font-size:11px;color:#888;text-transform:uppercase;letter-spacing:1px;font-weight:700;">Tipo de Solicitação</p>
                    <p style="margin:0;font-size:20px;font-weight:800;color:#1a1a1a;">💰 Financiamento de Veículo</p>
                  </td>
                </tr>
              </table>
            </td>
              ...
              <table width="100%" cellpadding="0" cellspacing="0">
                ...
                <tr>
                  <td style="padding:10px 0;">
                    <span style="font-size:12px;color:#888;display:block;margin-bottom:2px;">💵 Valor do Veículo</span>
                    <span style="font-size:16px;color:#c5a059;font-weight:600;">${escapeHtml(valorVeiculo)}</span>
                  </td>
                </tr>
                ` : ''}
              </table>
          ...
          <tr>
            <td style="padding:28px 30px 0;text-align:center;">
              <a href="mailto:${escapeHtml(email)}?subject=Re: Solicitação de Financiamento&body=Olá ${encodeURIComponent(nome)},%0A%0A"
  ...
  try {
    ...
    if (!nome || !email || !telefone || !mensagem) {
      return NextResponse.json(
        { sucesso: false, erro: 'Nome, email, telefone e mensagem são obrigatórios' },
        { status: 400 }
      );
    }
